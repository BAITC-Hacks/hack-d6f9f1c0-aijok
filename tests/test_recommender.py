...
def test_returns_three_results_in_stable_order(contractors):
    ...
    assert [item["explanation"] for item in first["results"]] == [item["explanation"] for item in second["results"]]
    assert all("Приветствую" not in item["explanation"] for item in first["results"])
...
def test_accepts_display_date_and_regular_integer_budgets(contractors):
    for budget in (150_000, 200_000, 800_000, 1_000_000):
        normalized = validate_request(
            {"city": "Алматы", "event_date": "25.09.2026", "event_format": "свадьба", "category": "Фотограф", "budget": str(budget)},
            categories_from(contractors),
        )
        assert normalized["event_date"] == date(2026, 9, 25)
        assert normalized["budget"] == budget
