...
def validate_request(params: dict[str, Any], categories: list[str]) -> dict[str, Any]:
    ...
    raw_date = str(params["event_date"]).strip()
    try:
        event_date = date.fromisoformat(raw_date) if "-" in raw_date else datetime.strptime(raw_date, "%d.%m.%Y").date()
...
def _description_detail(contractor: Contractor) -> str:
    """Select a useful, deterministic profile detail instead of an introductory phrase."""
    cleaned = re.sub(r"[•\n]+", ". ", contractor.description)
    sentences = [part.strip(" -–—:;") for part in re.split(r"(?<=[.!?])\s+", cleaned) if part.strip()]
    empty_openers = ("привет", "всем привет", "приветствую", "меня зовут", "добрый день", "здравствуйте")
    keywords = {
        "опыт": 4,
        "специал": 4,
        "стиль": 3,
    ...
    for position, sentence in enumerate(sentences):
        ...
        if lowered.startswith(empty_openers) or len(sentence) < 25:
            continue
        score = sum(weight for keyword, weight in keywords.items() if keyword in lowered)
        candidates.append((-score, position, sentence))
    if not candidates:
        return ""
    detail = min(candidates)[2]
    detail = re.sub(r"^(я\s*[,—-]?\s*)", "", detail, flags=re.IGNORECASE).strip()
    return detail[:220].rstrip(" ,;:") + ("…" if len(detail) > 220 else "")
...
def _explanation(contractor: Contractor, query: dict[str, Any]) -> str:
    ...
    sentences = [
        f"Подрядчик работает в городе {contractor.city} с форматом «{query['event_format']}»: цена от {price} ₸ оставляет запас {reserve} ₸ от бюджета."
    ]
    conditions = []
    if query["language"]:
        if query["language"] in contractor.languages:
            conditions.append(f"выбранный {query['language']} язык поддерживается")
        else:
            conditions.append(f"выбранный {query['language']} язык не указан; в профиле отмечены: {', '.join(contractor.languages)}")
    else:
        conditions.append(f"доступные языки: {', '.join(contractor.languages)}")
    if query["duration"] is not None:
        if contractor.max_hours is None:
            conditions.append("для этой услуги ограничение по длительности не применяется")
        else:
            conditions.append(f"лимит {contractor.max_hours:g} ч покрывает запрошенные {query['duration']:g} ч")
    elif contractor.max_hours is not None:
        conditions.append(f"профиль рассчитан на работу до {contractor.max_hours:g} ч")
    sentences.append(conditions[0].capitalize() + ("; " + "; ".join(conditions[1:]) if len(conditions) > 1 else "") + ".")
    detail = _description_detail(contractor)
    if detail:
        sentences.append(f"В описании профиля указано: {detail.rstrip('.')}.")
